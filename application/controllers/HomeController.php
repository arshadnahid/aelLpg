...
class HomeController extends CI_Controller {
    ...
    public function moduleDashboard() {
        //check company summary access show or hide
        ...
        //check company today summary access show or hide
        $condition = array(
            'admin_id' => $this->admin_id,
            'navigation_id' => '105',
        );
        ...
        foreach ($allProduct as $key => $value) {
            $productStck = $this->Dashboard_Model->getInventoryStock($this->dist_id, $value->product_id);
            if (!empty($productStck)) {
                $allProductStock[] = $productStck . '-' . $value->productName;
            }
        }
